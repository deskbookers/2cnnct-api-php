...
class API_2cnnct_API
{
     ...
     */
    public static function getLoginUrlForResellerCollection(
        $apiHost,
        $resellerCollectionId,
        $returnUrl,
        $apiVersion = 1,
        $locale = null
    ) {
        ...
        $data = array_map(
            'json_encode',
            Arr::merge(static::$extraData_, [
                'resellerCollectionID' => (int) $resellerCollectionId,
        ...
        // URI
        $uri = '/api/v' . $apiVersion . '/getLoginResellerCollectionUrl?'
            . http_build_query($data)
        ;

        // Make request through curl
        $ch = static::prepareCurl();
        $url = static::prepareApiHost($apiHost, $ch) . $uri;
        $sentHeaders = (
        ...
        if (! is_object($json)) {
            ksort($curlInfo);
            Logger::error(
                '[APIC-1001] Invalid API response format',
                null,
                [
                    'response' => $response,
                    'url' => $url,
            ...
            throw new API_2cnnct_CallException(
                500,
                '[APIC-1002] Invalid API response format'
            );
        } else {
            unset($response);
            if (property_exists($json, 'error') && $json->error) {
                throw new API_2cnnct_CallException(
                    property_exists($json, 'errorCode')
                        ? $json->errorCode
                        : 500,
                    property_exists($json, 'errorMessage')
                        ? $json->errorMessage
                        : 'Error',
                    property_exists($json, 'errorTrace')
                        ? $json->errorTrace
                        : null,
                    property_exists($json, 'extra')
                        ? (array) $json->extra
                        : null
                );
                ...
                ksort($curlInfo);
                Logger::error(
                    '[APIC-1003] Invalid API response format',
                    null,
                    [
                        'json' => $json,
                        'url' => $url,
                ...
                throw new API_2cnnct_CallException(
                    500,
                    '[APIC-1004] Invalid API response format'
                );
            }
        }
     ...
     * @param resource $ch
     */
    public static function prepareApiHost(
        $host,
        $ch = null,
        array& $headers = null
    ) {
        ...
        $mapping = Arr::get(
            (array) Kohana::$config->load('api.hostMappings'),
            $host
        );
        if ($mapping) {
    ...
    private function buildCheckHash($method, $timestamp, $uri, array $post)
    {
        return hash_hmac(
            'sha512',
            $this->buildCheckData($method, $timestamp, $uri, $post),
            $this->privateKey_
        );
    }
    ...
    protected function buildUri($uri, array $uriParams = null)
    {
        foreach ((array) $uriParams as $key => $value) {
            if (strlen($value) == 0) {
                throw new Exception(
                    '[APIC-1005] Invalid uri param \'' . $key
                    . '\' given, can not be empty'
                );
            }
    ...
    protected function post_(array $data, $uri, array $uriParams = null)
    {
        ...
        // Make request through curl
        $ch = static::prepareCurl();
        $url = static::prepareApiHost($this->apiHost_, $ch, $headers) . $uri;
        $sentHeaders = Arr::merge(
            $headers,
            [
                'Timestamp: ' . ($timestamp = time()),
                'Authenticate: ' . $this->publicKey_ . ':'
                    . $this->buildCheckHash('POST', $timestamp, $uri, $data),
            ],
            static::$referer_ === null ? [] : [
                'Referer: ' . static::$referer_,
            ]
        ...
        if (! is_object($json)) {
            ksort($curlInfo);
            Logger::error(
                '[APIC-1006] Invalid API response format',
                null,
                [
                    'response' => $response,
                    'url' => $url,
            ...
            throw new API_2cnnct_CallException(
                500,
                '[APIC-1007] Invalid API response format'
            );
        } else {
            unset($response);
            if (property_exists($json, 'error') && $json->error) {
                ...
                ksort($curlInfo);
                Logger::error(
                    '[APIC-1008] Invalid API response format',
                    null,
                    [
                       'json' => $json,
                        'url' => $url,
                ...
                throw new API_2cnnct_CallException(
                    500,
                    '[APIC-1009] Invalid API response format'
                );
            }
        }
    ...
    public function getCached(
        ...
        if ($cache === null) {
            throw new Exception(
                '[APIC-1010] No valid API Cache instance is provided'
            );
        }

        // Cache name
        $ch = static::prepareCurl();
        $url = static::prepareApiHost($this->apiHost_, $ch, $headers)
        ...
        if ($cache->hasCache($cacheName, $lifetime)) {
            $response = $cache->getCache($cacheName, $lifetime);
        ...
        $this->logs_[count($this->logs_) - 1]['curlInfo']['cached'] = true;
     ...
     * @return array|View
     */
    public function logs($formatted = false)
    {
        if ($formatted) {
            return View::factory('api/logs')
                ->set('api', $this)
            ;
        }
     ...
     * @param array $headers Received headers
    ...
    protected function get_(
        ...
        $sentHeaders = Arr::merge(
            ...
            [
                'Timestamp: ' . ($timestamp = time()),
                'Authenticate: ' . $this->publicKey_ . ':'
                    . $this->buildCheckHash('GET', $timestamp, $uri, []),
            ],
            static::$referer_ === null ? [] : [
                'Referer: ' . static::$referer_,
            ]
        ...
        if (! is_object($json)) {
            ksort($curlInfo);
            Logger::error(
                '[APIC-1011] Invalid API response format',
                null,
                [
                    'url' => $url,
                    'response' => $response,
            ...
            throw new API_2cnnct_CallException(
                500,
                '[APIC-1012] Invalid API response format'
            );
        } else {
            unset($response);
            if (property_exists($json, 'error') && $json->error) {
                ...
                ksort($curlInfo);
                Logger::error('[APIC-1013] Invalid API response format',
                    null,
                    [
                        'json' => $json,
                        'url' => $url,
                ...
                throw new API_2cnnct_CallException(
                    500,
                    '[APIC-1014] Invalid API response format'
                );
            }
        }
